...
import { renderPlantUml } from '../modules/plantUml.js'
...
watchEffect(async () => {
  try {
    loading.value = true
    svg.value = await renderPlantUml(`${props.server}/${isDark.value ? 'd' : ''}svg/${props.code}`)
  } catch (error: any) {
...
function handleUpdate($event: ShadowRoot) {
  nextTick(() => {
    ...
    if (svgEl) {
      ...
      if (props.scale != null && actualHeight.value != null && actualWidth.value != null) {
