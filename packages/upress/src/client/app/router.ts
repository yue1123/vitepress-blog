...
import { inBrowser, notFoundPageData } from '../../shared/shared'
import type { PageData, PageDataPayload, Awaitable } from '../../shared/shared'
  ...
  async function loadPage(href: string, scrollPosition = 0, isRetry = false) {
    ...
    try {
      ...
      if (latestPendingPath === pendingPath) {
        ...
        route.data = import.meta.env.PROD ? markRaw(__pageData) : (readonly(__pageData) as PageData)
...
function handleHMR(route: Route): void {
  ...
  if (import.meta.hot) {
    // hot reload pageData
    import.meta.hot!.on('upress:pageData', (payload: PageDataPayload) => {
